...
class WIN(Screen):
    def __init__(self, **kwargs):
        ...
        self.source_installer = "/Users/leo/Downloads/Win11_24H2_En_x64.iso"
        self.target_volume = "/Volumes/WINDOWS"
        self.target_volume_label = "WIN"
        self.target_volume_format = "MS-DOS"
        self.target_volume_scheme = "gpt"        
        self.bypass_tpm = True
        self.mounted_iso_volume = None
    ...
    def on_enter(self):
        ...
        self.ids.target_volume.text = self.target_volume
        self.ids.target_volume_label.text = self.target_volume_label
        self.ids.target_volume_format.text = self.target_volume_format
        self.ids.target_volume_scheme.text = self.target_volume_scheme
    ...
    def main_execute(self):
        ...
        try:
            ...
            if format_stt:
                ...
                if mounted_volume:
                    if self.target_volume_format.lower() == 'ms-dos':
                        install_wim_path = os.path.join(
                            mounted_volume, "sources", "install.wim"
                        )
                        if os.path.exists(install_wim_path):
                            ...
                            if install_wim_size < 4:
                                self.copy_installer(mounted_volume, self.target_volume)
                            else:
                                self.split_install_wim(mounted_volume, self.target_volume)
                                self.copy_installer(
                                    mounted_volume, self.target_volume, exclude_wim=True
                                )
                    else:
                        self.copy_installer(mounted_volume, self.target_volume)
                    if self.bypass_tpm == True:
                        mac.copy_file(os.path.join(self.app.exe_path,"autounattend.xml"), self.target_volume)
                        self.update_output_label("Bypass TPM!")
                    self.unmount_iso(mounted_volume)
                else:
    ...
    def format_volume(self, target):
        ...
        try:
            format_cmd = ["diskutil", "eraseVolume", self.target_volume_format, mac.sanitize_label(self.target_volume_label), target]
    ...
    def mount_iso(self, iso_path):
        ...
        try:
            ...
            for line in result.splitlines():
                if "/Volumes/" in line:
                    ...
                    self.mounted_iso_volume = mounted_volume
        ...
        return None
    def unmount_iso(self, iso_path):
        if iso_path:
            self.update_output_label(f"unmounting ISO..")
            try:
                result = subprocess.check_output(["hdiutil", "unmount", iso_path]).decode()
                print(result.strip())
                self.update_output_label(f"Unmounted ISO!")
            except subprocess.CalledProcessError as e:
                self.update_output_label(f"Failed to mount ISO: {e}")
            return None
    def copy_installer(self, source, target, exclude_wim=False):
        try:
            ...
            self.execute_command(
                rsync_cmd,
                f"Copying data from ISO to: {target} {'excluding wim file ' if exclude_wim else ''}",
            )
        except subprocess.CalledProcessError as e:
            self.update_output_label(f"rsync failed: {e}")
    ...
    def split_install_wim(self, source, target):
        ...
        try:
            ...
            split_cmd = [
                ...
                "split",
                install_wim,
                os.path.join(wim_source_dir, "install.swm"),
                "3800",
            ]
            print(f"split cmd: {split_cmd}")
            self.execute_command(
                split_cmd, f"Splitting install.wim and copied to {target}"
            )
        except subprocess.CalledProcessError as e:
    ...
    def set_text(self, text, target="source"):
        ...
        if text is not None:
            if target == "source":
                ...
                self.target_volume_label = text
            elif target == 'format':
                self.target_volume_format = text
            elif target == 'scheme':
                self.target_volume_scheme = text
    ...
    def set_path(self,target="target"):
        ...
        if p != "":
            if target == "target":
                ...
                self.ids.target_volume.text = p
                self.ids.target_volume.focus = True
                print(p)
            elif target == "source":
                self.source_installer = p
                self.ids.source_installer.text = p
                self.ids.source_installer.focus = True
                print(p)
